  ...
  switch(method) {
    ...
    case 'GET':
      try {
        ...
        const [rows, fields] = await connection.execute('SELECT beneficiaries.id_beneficiary, beneficiaries.fullname, beneficiaries.hampers, beneficiaries.age, beneficiaries.gender, beneficiaries.delivered_at, users.username FROM beneficiaries INNER JOIN users ON beneficiaries.id_user=users.id_user WHERE id_beneficiary = ?', [id])
